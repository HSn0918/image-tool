...
- `wasm/image-wasm` 目录下包含了基于 `wasm-bindgen` 的图片缩放核心，等比例缩放工具会优先调用该模块来完成像素级计算，再自动回退到 Canvas 方案。
- 修改 Rust 代码后执行 `bun run wasm:build` 以重新生成 `pkg/` 内的 JS/wasm 绑定（`bun run build` 会自动触发该步骤）。
