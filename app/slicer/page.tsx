...
export default function SlicerPage() {
    ...
    const updateGridOverlay = useCallback(() => {
        ...
        for (let c = 1; c < cols; c++) {
            ...
            v.className =
                "absolute border-l border-dashed border-white h-full top-0 drop-shadow-[1px_0_0_rgba(0,0,0,0.8)]";
        ...
        for (let r = 1; r < rows; r++) {
            ...
            h.className =
                "absolute border-t border-dashed border-white w-full left-0 drop-shadow-[0_1px_0_rgba(0,0,0,0.8)]";
    ...
    const onDownload = async () => {
        ...
        const zip = new JSZip();
        const createBoundaries = (size: number, segments: number) =>
            Array.from({length: segments + 1}, (_, i) =>
                i === segments ? size : Math.floor((i * size) / segments),
            );
        const colBounds = createBoundaries(canvas.width, colsVal);
        const rowBounds = createBoundaries(canvas.height, rowsVal);
        const tileCanvas = document.createElement("canvas");
        const ctx = tileCanvas.getContext("2d");
        if (!ctx) return;
        for (let r = 0; r < rowsVal; r++) {
            for (let c = 0; c < colsVal; c++) {
                const sx = colBounds[c];
                const sy = rowBounds[r];
                const sw = Math.max(1, colBounds[c + 1] - sx);
                const sh = Math.max(1, rowBounds[r + 1] - sy);
                tileCanvas.width = sw;
    ...
    return (
        <div className="flex h-full flex-col gap-6">
            ...
            <div className="grid min-h-0 flex-1 gap-6 lg:grid-cols-[1fr_320px]">
                ...
                <div
                    ref={dropZoneRef}
                    className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-dashed bg-muted/20 p-6 transition-colors hover:bg-muted/30"
                ...
                >
                    ...
                    <div
                        className={`relative h-full w-full overflow-hidden rounded-md border bg-background ${hasImage ? "block" : "hidden"}`}
                    >
